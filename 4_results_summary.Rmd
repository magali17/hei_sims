...
saveRDS(label_pollutant, file.path("Output", "label_pollutant_fn.rda"))
...
# uk predictions
predictions0 <- readRDS(file.path("Output", "UK Predictions", "all_predictions.rda")) %>% 
  
  # --> 
  # drop 2 seasons SI analysis
  #filter(design != "two seasons") %>%
  ...
  select(
    spatial_temporal, design, version, visits, sites, total_stops, campaign, variable, out_of_sample, location, prediction, var1.var, 
    reference, estimate 
  )
...
grid_predictions0 <- readRDS(file.path("Output", "UK Predictions", "grid_predictions.rda"))  %>%
  ...
  summarize(
    ...
    median_var1.var = median(var1.var)
  ) %>%
...
### --> Note the total # of stops is slightly lower for the fewer total stops 24+ visits b/c some sites don't have that many visits
...
```{r}
# note: restart Rstudio if have map issues  
...
m1 <- map0 + 
  ...
  #monitoring stops
  geom_sf(data = st_transform(loc_lat_long, project_crs), aes(col = "Training-Validation Set"), inherit.aes = F, size=2) + 
  ...
  labs(x = "Longitude", y = "Latitude", col = "Sites", fill="") 
...
anova1 <- lapply(group_split(stops, variable),   function(x) {
  ...
  mutate(
    ...
    factor = factor(factor, levels = c("Site", "Season", "Day", "Hour", "Error")),
    kind = ifelse(grepl("Site", factor), "Spatial",
                  ifelse(grepl("Day|Hour|Season", factor), "Temporal", "Residual")),
    kind = factor(kind, levels = c("Spatial", "Temporal",  "Residual"))
  ) %>%  
  label_pollutant()
...
anova1 %>%
  ggplot(aes(x=kind, y=percent, fill=factor)) + 
...
#ggsave(file.path(image_path, "SI", paste0("anovas.png")), width = 9, height = 5)
...
prediction_diff %>%
  ...
  mutate(version = ifelse(version == "business & rush", "business \n& rush", as.character(version)),
         version = ifelse(version == "all training data", "", version),
         version = factor(str_to_title(version), levels = c("", str_to_title(version_levels), "Business \n& Rush" )),
         design = ifelse(design=="full", "All-Data", str_to_title(design)),
         design = factor(design, levels = c("All-Data", "Fewer Total Stops", "Balanced Seasons", "Fewer Days", "Fewer Hours")),) %>% 
...
t0 <- model_eval %>%
  ...
  group_by(variable) %>%
  summarize(label = paste0("Test: RMSE = ", RMSE[2], ", ", "R2 = ", MSE_based_R2[2],"\n",  "Cross-Validation: RMSE = ", RMSE[1], ", ","R2 = ", MSE_based_R2[1] #, #"\n",
...
predictions %>%
  ...
  mutate(out_of_sample = ifelse(out_of_sample=="CV", "Cross-Validation", "Test")) %>%
  ...
  geom_text(data= t0, aes(x=Inf, y=-Inf, label=label), size=3, inherit.aes = F, hjust=1, vjust=-0.1 ) +
...
predictions %>%
  filter(grepl("gs_", reference),
         ...
         (design == "fewer total stops" & version == "3000") |
           (design == "balanced seasons" & version %in% c("2")) |
           (design == "fewer hours" & version %in% c("rush", "business")) |
           #design == "fewer days" |
           design == "full"
         ) %>% #distinct(design, version) 
  mutate(
    version = ifelse(version =="2", "2 Seasons",
                     ifelse(version =="3000", "3,000 Total Stops",
                            ifelse(version =="rush", "Rush Hours",
                                   ifelse(version =="business", "Business Hours",
                                          ifelse(version =="all training data", "All-Data", NA))))),
    version = factor(version, levels = c("All-Data", "3,000 Total Stops", "2 Seasons", "Business Hours", "Rush Hours"))
    ) %>% # distinct(version, version2)
  
  ggplot(., aes(x=estimate, y=prediction, col=version)) +
  facet_wrap_equal(~variable, scales="free") +
  theme(aspect.ratio=1) +
  # keeping SE=F b/c all-data estimates have fewer data points, which results in an artificially wider SE
  geom_abline(slope = 1, intercept = 0, linetype=2, alpha=0.5) +
  geom_point(alpha=0.04, shape=21) +
  #geom_line(stat="smooth", method= "lm", alpha=0.6) +
  geom_smooth(se=T, formula = "y~x", method="lm", size=0.6) +
  #geom_label(inherit.aes = F, aes(x=Inf, y=Inf),  hjust = 1, vjust=1, label="1:1", size=3) +
  geom_abline(slope = c(1.25, 0.75), intercept = 0, linetype=2, alpha=0.5) +
  labs(col="") 

ggsave(file.path(image_path, "SI", "gs_scatterplot_example_designs.png"), width = 9, height = 7)
...
for (i in seq_along(eval_cols[1:2])) {
  ...
  p <- model_eval %>%
    ...
    mutate(
      out_of_sample = ifelse(out_of_sample=="CV", "Cross-Validation", "Test"),
      version = factor(str_to_title(version), levels = c(str_to_title(version_levels), "Business \n& Rush" )),
      design = factor(str_to_title(design), levels = c("Full", "Fewer Total Stops", "Balanced Seasons", "Fewer Days", "Fewer Hours")),
...
for (i in plot_vars) {
  ...
  temp <- site_visit_summary %>%
    ...
    mutate(sites = as.numeric(as.character(sites)),
           ...
           reference = ifelse(grepl("gs_", reference), "All-Data (Best) Estimate", "Reduced Sampling Estimate"),
  ...
  if(grepl("RMSE", i)) {p_image_path = file.path(image_path, "SI")
  } else{p_image_path = image_path}
...
p <- normalized_model_eval %>%
  ...
  mutate(model_eval = ifelse(grepl("RMSE", model_eval), "Normalized RMSE", "Normalized R2" ),
         ...
         reference = ifelse(grepl("gs_", reference), "All-Data\n(Best)\nEstimate", "Reduced\nSampling\nEstimate")
         ) %>% 
...
p + geom_smooth(stat = 'summary', fun.data = median_hilow, fun.args = list(conf.int = 0.5), alpha = 0.2)
...
ggsave(file.path(image_path, paste0("campaign_estimates_model_eval2_NoIQR.png")), width = 10, height = 6)
...
# dt = difference_map_data
# var_names. = unique(full_map_data$variable)
# x= "PNC (pt/cm3)"
# map_label = "Prediction" 
map_predictions <- function(dt, var = "prediction_median", map_type="", new_text_size="", var_names.=var_names, pt_size=1, map_label = "Prediction", ...) {
  ...
  prediction_maps <- lapply(var_names., function(x) {
    ...
    p <- filter(dt, variable == x) %>%
      ...
      guides(colour = guide_colourbar(title.position="top", barwidth = 5))
    ...
    if(map_type=="prediction_iqr"){
      p <- p + theme(strip.background.x = element_blank(), strip.text.x = element_blank())
    }
  ...
  prediction_maps %>% 
    ggarrange(plotlist = ., ncol = length(var_names.), align = "hv") %>%
    annotate_figure(., left = ggpubr::text_grob(map_label, face = "bold", rot=90))
}
...
full_map_data <- df0 %>%
  filter(grepl("Full", design)) %>%
  mutate(design = "Temporally-Balanced",
         prediction_median = ifelse(grepl("PNC", variable), prediction_median/10^3, prediction_median),
         variable = ifelse(grepl("PNC", variable), "PNC (1k pt/cm3)", as.character(variable)),
         variable = factor(variable, levels = c("PNC (1k pt/cm3)", "BC (ng/m3)", "NO2 (ppb)", "PM2.5 (ug/m3)", "CO2 (ppm)"))
         ) 

# Median site prediction difference relative to the full campaign 
difference_map_data <- df0 %>%
  filter(!grepl("Full", design)) %>%
  
  mutate(
    design = ifelse(design == "3000 Stops", "Temporally-Balanced",
                    ifelse(design == "2 Season", "2 Seasons",
                           ifelse(design == "Business Hours", "Business Hours",
                                  ifelse(design == "Rush Hours", "Rush Hours", NA
                           )))),
    design = relevel(factor(design), ref = "Temporally-Balanced"),
    median_prediction_diff = ifelse(grepl("PNC", variable), median_prediction_diff/10^3, median_prediction_diff),
    variable = ifelse(grepl("PNC", variable), "PNC (1k pt/cm3)", as.character(variable)),
    variable = factor(variable, levels = c("PNC (1k pt/cm3)", "BC (ng/m3)", "NO2 (ppb)", "PM2.5 (ug/m3)", "CO2 (ppm)"))
    ) 
...
ggarrange(full_map, difference_map, ncol = 1, heights = c(1.2,4))

#ggsave(file.path("~", "Desktop", paste0("maps_of_prediction_diff.png")),  height = 12, width = 7)
ggsave(file.path(image_path, paste0("maps_of_prediction_diff.png")),  height = 12, width = 7)
...
iqr_map_data <- df0 %>%
  ...
  mutate(
    ...
    design = relevel(factor(design), ref = "Temporally-Balanced"),
    prediction_iqr = ifelse(grepl("PNC", variable), prediction_iqr/10^3, prediction_iqr),
         variable = ifelse(grepl("PNC", variable), "PNC (1k pt/cm3)", as.character(variable)),
         variable = factor(variable, levels = c("PNC (1k pt/cm3)", "BC (ng/m3)", "NO2 (ppb)", "PM2.5 (ug/m3)", "CO2 (ppm)"))
  ) 
  
iqr_map <- iqr_map_data %>% 
  map_predictions(., var = "prediction_iqr",
                  map_type = "prediction_iqr",
                  map_label = "Prediction IQR (3k Stops)",
                  var_names. = unique(full_map_data$variable)
                  )
                  
 ggarrange(full_map, iqr_map, ncol = 1, heights = c(1.2,4))

ggsave(file.path(image_path, "SI", paste0("maps_of_prediction_iqr.png")),  height = 12, width = 7)

```

- same as above for prediction variance (var1.var)


```{r}
var1.var_map <- iqr_map_data %>%
  map_predictions(., var = "median_var1.var",
                  map_type = "prediction_iqr",
                  map_label = "Prediction Variance (3k Stops)",
                  var_names. = unique(full_map_data$variable)
                  )

 ggarrange(full_map, var1.var_map, ncol = 1, heights = c(1.2,4))

ggsave(file.path(image_path, "Other", paste0("maps_of_prediction_var1.var.png")),  height = 12, width = 7)

```

median site prediction vs median var1.var value by design-version and variable
...
predictions %>%
  ...
  group_by(design, variable, version, location) %>%
  summarize(
    prediction = median(prediction),
    var1.var = median(var1.var)
  ) %>% 
  #mutate(design = paste(design, ", ", version)) %>%

  ggplot(aes(x=prediction, y=var1.var, 
             group=version, 
             col=design, #shape = design
             )) + 
  facet_wrap(~variable, scales = "free") + 
  ...
  labs(
    x = "median site prediction",
    y = "median site var1.var",
    caption = "lines connect design-versions; colors are by design"
  )
  
ggsave(file.path(image_path, "Other", paste0("prediction_vs_var1.var.png")),  height = 8, width = 8)
...
-for poster (set to eval=F for now)

```{r, fig.show='hide', eval=F}
reduce_size <- 0.88
...
pnc_full_map <- df0 %>%
  ...
  guides(colour = guide_colourbar(barwidth = barwidth, barheight = barheight)) +
...
pnc_sample_maps <- df0 %>%
  filter(!grepl("Full|Season", design),
         grepl("PNC", variable),
         ) %>% 
  mutate(
    design = ifelse(design == "3000 Stops", "Time\nBalanced",
                    ifelse(design == "2 Season", "During\n2 Seasons\n",
                           ifelse(design == "Business Hours", "During\nBusiness Hrs",
                                  ifelse(design == "Rush Hours", "During\nRush Hrs", NA
                           )))),
    
    design = relevel(factor(design), ref = "Time\nBalanced"),
  ...
  guides(colour = guide_colourbar(barwidth = barwidth, barheight = barheight)) +
  labs(title = "Prediction Difference (3k stops)",
       col = "1k\npt/cm3"
       )  
...
ggarrange(pnc_full_map, pnc_sample_maps, 
  ...
  annotate_figure(top = text_grob("Mobile Monitoring Design (Ultrafine Particles)", size = 6, face = "bold"))

ggsave(file.path(image_path, "toc_graphic.png"), width = fig_w, height = fig_h, bg = "white"
       )
